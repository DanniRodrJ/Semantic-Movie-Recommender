...
from core.models import Movie, Actor, PipelineAnalytics
...
def fetch_and_vectorize_movies(pages=3, language='en-US'):
    ...
    total_cost = (total_pipeline_tokens / 1000.0) * COST_PER_1K_TOKENS
    
    PipelineAnalytics.objects.create(
        task_name="fetch_and_vectorize_movies",
        movies_processed=movies_processed,
        total_tokens_used=total_pipeline_tokens,
    ...
    success_msg = f"✅ Pipeline finished: {status_msg}. Movies: {movies_processed} new movies added and vectorized | Tokens: {total_pipeline_tokens} | Cost: ${total_cost:.4f} | Time: {execution_time_sec:.1f}s"
